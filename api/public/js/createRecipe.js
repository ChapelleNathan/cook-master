export async function createRecipe() {
    ...
    try {
        ...
        const createMarkup = domUtils.createMarkup;
        ...
        //get units
        const units = [
            { "name": "UNIT_GRAM", "alias": "grammes" },
            { "name": "UNIT_PACK", "alias": "packets" },
            { "name": "UNIT_OBJECT", "alias": "unité" },
            { "name": "UNIT_LITER", "alias": "litre" }
        ];
        const gastronomies = [
            { "name": "french", "alias": "Française" },
            { "name": "chinese", "alias": "Chinoise" },
        ];
        ...
        function addOptions(selectElement,optionData) {
            optionData.forEach(unit => {
                createMarkup("option", unit.alias, selectElement, [{ value: unit.name }])
            })
        }
        ...
        rmIngredientBtn.setAttribute("disabled", "true")
        addOptions(ingredient1UnitSelect,units)
        addOptions(gastronomySelect,gastronomies)
    } catch (error) {
        console.error(error)
    }
